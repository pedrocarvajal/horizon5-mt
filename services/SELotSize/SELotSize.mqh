	...
	double CalculateByStopLoss(double nav, double stopLossDistance, double equityAtRisk) {
		if (nav <= 0) {
			logger.Warning(
				...
				"lot size calculation invalid | field=nav reason='required'"
			);
			return 0.0;
		}
		...
		if (stopLossDistance <= 0) {
			logger.Warning(
				LOG_CODE_CONFIG_INVALID_PARAMETER,
				"lot size calculation invalid | field=stop_loss_distance reason='required'"
			);
			return 0.0;
		}
		...
		if (equityAtRisk <= 0) {
			logger.Warning(
				LOG_CODE_CONFIG_INVALID_PARAMETER,
				"lot size calculation invalid | field=equity_at_risk reason='required'"
			);
			return 0.0;
		}
