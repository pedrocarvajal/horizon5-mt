...
void SRImplementationOfHorizonGateway::handleGetKlines(SGatewayEvent &event) {
	logger.Info(
	...
	if (event.symbol == "" || event.timeframe == "" || event.fromDate == "" || event.toDate == "") {
		ackServiceEventError(event, "symbol, timeframe, from_date and to_date are required");
		return;
	}

	ENUM_TIMEFRAMES period = MapTimeframe(event.timeframe);
	if (period == PERIOD_CURRENT) {
		ackServiceEventError(event, "Invalid or unsupported timeframe");
		return;
	}
	...
	if (copied <= 0) {
		ackServiceEventError(event, "No kline data available for the requested range");
		return;
	}
	...
	if (fileHandle == INVALID_HANDLE) {
		ackServiceEventError(event, "Failed to create CSV file");
		return;
	}
	...
	if (readHandle == INVALID_HANDLE) {
		...
		ackServiceEventError(event, "Failed to read CSV file for upload");
		return;
	}
	...
	if (uploadedFileName == "") {
		ackServiceEventError(event, "Failed to upload CSV file");
		return;
	}
