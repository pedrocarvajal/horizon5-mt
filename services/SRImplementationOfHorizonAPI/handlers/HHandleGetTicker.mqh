#ifndef __H_HANDLE_GET_TICKER_MQH__
#define __H_HANDLE_GET_TICKER_MQH__

void HandleGetTicker(SHorizonEvent &event, HorizonAPI &api, SELogger &eventLogger) {
	eventLogger.Info(StringFormat("Event received | %s | id=%s", event.key, event.id));
	if (event.symbol == "") {
		AckServiceEventError(event, api, eventLogger, "No symbols provided");
		return;
	}
